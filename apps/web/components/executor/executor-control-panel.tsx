...
export const ExecutorControlPanel = memo(
  function ExecutorControlPanelComponent() {
    ...
    const handleAutoExecuteToggle = useCallback(
      ...
      [toggleAutoMutation, toast]
    );

    const handleMaxPositionsChange = useCallback(
      (value: string) => {
    ...
    const handleAlgorithmicToggle = useCallback(
      (checked: boolean) => {
        updateConfigMutation.mutate(
    ...
    const handleExchangeCredentialsChange = useCallback(
      (value: string) => {
        updateConfigMutation.mutate(
      ...
      [updateConfigMutation, credentials, toast]
    );

    // Memoize selected credential exchange
        ...
        <CardContent className="space-y-6">
          ...
          {config.mode === "LIVE" && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                <strong>Warning:</strong> LIVE mode is active. Real orders will
                be placed on exchanges with real funds.
              </AlertDescription>
            </Alert>
          )}
