...
import { ServiceConstants, ServicePorts, ServiceUrlResolver } from "./config";
...
export class ServiceClient {
  ...
  private getCircuitBreaker(serviceName: string): CircuitBreaker {
    ...
    const breaker = new CircuitBreaker({
      failureThreshold: ServiceConstants.CIRCUIT_BREAKER.FAILURE_THRESHOLD,
      successThreshold: ServiceConstants.CIRCUIT_BREAKER.SUCCESS_THRESHOLD,
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      if (!data.success) {
        throw new AppError(
          data.error?.code ?? ErrorCode.EXTERNAL_SERVICE_ERROR,
  ...
  private executeWithProtection<T>(
    ...
    if (this.enableRetry && retryable) {
      return retryWithBackoff(executeRequest, {
        ...
        backoffFactor: ServiceConstants.RETRY.BACKOFF_FACTOR,
